    ...
    <div className={cn("flex items-center gap-3", className)}>
      {links.map(({ label, href, icon: Icon }) => (
        <a
          ...
          aria-label={label}
          className="group relative grid h-10 w-10 place-items-center rounded-full border border-primary/35 bg-surface/60 text-muted shadow-[0_0_18px_-6px_color-mix(in_srgb,var(--color-primary)_60%,transparent)] ring-1 ring-inset ring-white/10 transition-all duration-300 hover:-translate-y-1 hover:scale-110 hover:border-transparent hover:text-white hover:shadow-[0_10px_28px_-8px_color-mix(in_srgb,var(--color-primary)_85%,transparent)]"
        >
